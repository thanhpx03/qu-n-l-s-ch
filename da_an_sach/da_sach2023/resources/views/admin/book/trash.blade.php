...
@section('content')
    <div class="container-fluid">
        ...
        <!-- Page Heading -->
        <h1 class="h3 mb-2 text-gray-800">Thùng rác phim</h1>
        <!-- DataTales Example -->
                ...
                <div class="table-responsive">
                    <div id="dataTable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                        <div class="row">
                            ...
                            <div class="col-sm-12 col-md-6">
                                <form action="{{ route('book.trash') }}" method="get">
                                    <div class="row">
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <table class="table table-bordered dataTable no-footer" id="dataTable" width="100%"
                                    ...
                                    <tbody>
                                    @foreach($deleteItems as $movie)
                                        <tr class="odd">
                                            <td class="sorting_1 text-center">
                                                <label>
                                                    <input type="checkbox" class="child-checkbox" value="{{ $movie->id }}">
                                                </label>
                                            </td>
                                            ...
                                            <td class="text-center">
                                                <input type="checkbox" value="{{ $movie->status }}"
                                                       name="status" data-item-id="{{ $movie->id }}"
                                                       class="switch1 switch-status switchery-small"
                                                    {{ $movie->status == 1 ? 'checked' : '' }}/>
                                            </td>
                                            <td class="text-center">
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                        <a class="dropdown-item"
                                                           href="{{ route('book.restore',['id' => $movie->id]) }}">Khôi phục</a>
                                                        <a class="dropdown-item show_confirm"
                                                           href="{{ route('book.delete',['id' => $movie->id]) }}">Xóa vĩnh viễn</a>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
        ...
        function updateStatus() {
            $(document).ready(function () {
                $('.switch-status').change(function () {
                    ...
                    $.ajax({
                        method: 'POST',
                        url: '/admin/book/update-status/' + itemId,
                        data: {
                            _token: '{{ csrf_token() }}',
                            status: status
                        },
                    ...
                    if (selectedCheckboxes.length > 0) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                $.ajax({
                                    type: 'POST',
                                    url: '/admin/book/restoreSelected',
                                    data: {
                                        ids: selectedIds,
                                        _token: '{{ csrf_token() }}',
                                    },
                    ...
                    if (selectedCheckboxes.length > 0) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                $.ajax({
                                    type: 'POST',
                                    url: '/admin/book/permanentlyDeleteSelected', // Thay thế bằng tuyến đường xử lý xoá của bạn
                                    data: {
                                        ids: selectedIds,
                                        _token: '{{ csrf_token() }}',
                                    },
